...
void main()
{
    ...
    vec3 tangent = normalize(vec3(normalMatrix * aTangent));
    vec3 normal = normalize(vec3(normalMatrix * aNormal));
    tangent = normalize(tangent - dot(tangent, normal) * normal); //re-orthigonalize (gram-schmidt process)
    //vec3 bitangent = normalize(vec3(normalMatrix * aBitangent));
